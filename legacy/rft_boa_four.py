...
for year in ['2022','2021','2020','2019','2018','2017'][::-1]:
    for station in ['46001h','46076h','46077h','46080h','46083h']:
        ...
        raw_data=pd.read_csv('wave/'+station+year+'.txt.gz',delim_whitespace=True)
        # print(data)
        data=data[features]
        ...
        for s in np.arange(seeds-1,seeds):
            ...
            file_path = f'rft_wave_four_results//{year}//{station}//rftfourBOA{boat}{year}{station}'
        ...
        dfall.to_csv('wave/ABA/allp_rft_four_BOA'+str(Nls[0])+str(50)+year+station+'.csv')

        output_loc = f"rft_wave_four_results//{year}//{station}//"

        if not os.path.exists(output_loc):
            os.makedirs(output_loc)
            
        results_df = pd.DataFrame(results, columns=['year', 'station', 'test_rmse', 'test_mape', 'test_mase'])
        results_df.to_csv(f'rft_wave_four_results//{year}//{station}//results.csv')

        test_p=np.concatenate(test_pres_ea,axis=1)
        dfea=pd.DataFrame(test_p)
        dfea.to_csv(f'rft_wave_four_results//{year}//{station}//rftfourBOA{boat}{year}{station}')
